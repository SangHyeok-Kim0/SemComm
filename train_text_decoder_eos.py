"""Train ClipCap-style TextDecoder — EOS-aware variant.
Code
train_text_decoder.py 와 동일한 학습 파이프라인이지만, collate에서 각 caption
끝에 EOS 토큰을 붙여 모델이 자연 종료를 학습하도록 한다. 기존(EOS 미부착)
모델은 max_new_tokens 끝까지 채우며 LM pretraining junk(`_`, `<br />` 등)를
...
def make_collate(tokenizer, max_length: int):
    """EOS-aware collate. Per row: tokenize w/o specials, append EOS, then pad together.
    EOS는 caption 직후에 위치 — pad 토큰들은 EOS 뒤에서 attention=0으로 무시된다.
    """
    ...
    def collate(batch):
        ...
        ids_list = [
            tokenizer(t, truncation=True, max_length=max_length - 1,
                      add_special_tokens=False).input_ids + [eos_id]
            for t in texts
        ]
        enc = tokenizer.pad({"input_ids": ids_list}, padding=True,
                            return_tensors="pt")
...
def main():
    ...
    run_name = build_run_name(text_cfg, suffix="txt_eos")
